...
// https://github.com/anirudh985/scala-rake
// https://github.com/anirudh985/scala-rake/blob/master/src/main/scala/RAKE.scala
/**
...
object RAKE {
  ...
  private def calculateWordScores(listOfPhrases: List[List[String]]): Map[String, Double] = {
    ...
    val wordFreqMap: MapView[String, Int] = wordFreqList.groupBy(k => k._1).mapValues(l => l.map(_._2).sum)
    val wordDegreeMap: MapView[String, Int] = (wordFreqList ++ wordDegreeListPartial).groupBy(k => k._1).mapValues(l => l.map(_._2).sum)
